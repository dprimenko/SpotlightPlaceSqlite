...
public class PlacesAdapter extends ArrayAdapter<SpotPlace> implements ApiDAO.GmapsRequestStatus {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (view == null) {
            ...
            holder.imvItemEventsList = (ImageView) view.findViewById(R.id.imvItemEventsList);
            holder.imvUserItemEventsList = (CircleImageView) view.findViewById(R.id.imvUserItemEventsList);
            holder.txvTitleItemEventsList = (TextView) view.findViewById(R.id.txvTitleItemEventsList);
            holder.txvDescriptionItemEventsList = (TextView) view.findViewById(R.id.txvDescriptionItemEventsList);
            holder.txvAddressItemEventsList = (TextView) view.findViewById(R.id.txvAddressItemEventsList);
            holder.txvUserItemEventsList = (TextView) view.findViewById(R.id.txvUserItemEventsList);
            holder.txvPeopleEventsList = (TextView) view.findViewById(R.id.txvPeopleEventsList);
        ...
        holder.txvPeopleEventsList.setText(String.valueOf(item.getmNpeople().size()));
        return view;
    }
    ...
    @Override
    public void onErrorResponse(CharSequence messageError) {
            ...
            JsonArrayRequest getRequest = new JsonArrayRequest(Request.Method.GET, ApiDAO.URL_USER_BYID + getItem(params[0]).getmCreatorId(), null,
                    new Response.Listener<JSONArray>()
                    {
                        ...
                        public void onResponse(JSONArray response) {
                            ...
                            try {
                                nick = response.getJSONObject(0).getString("nick");
                                fullName = response.getJSONObject(0).getString("full_name");
                                profileImg = response.getJSONObject(0).getString("profile_img");
                            } catch (JSONException e) {
                                e.printStackTrace();
                            }
